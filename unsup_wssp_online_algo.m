function [I,dm]=unsup_wssp_online_algo(m,m_idx,k,log_s,weight_sq,varargin)
...
%   dm:     distance matrix (symmetric) of observed time series in m.
    ...
    if k == 1 
        ...
        if nargin > 3
            ...
            for i = 2:N
                for j = 1:(i-1)
                    ...
                    dm(i,j) = dist_ts_log(m(i,com_obs_ind),m(j,com_obs_ind),'cov',log_s,weight_sq);
                    dm(j,i) = dm(i,j);
                end
            end
        end
        ...
        for j = k:N
            ...
            I_selected = ...
                unsup_wssp_offline_algo(selected_obs, selected_obs_ind, k, log_s,weight_sq, dm(selected_idx,selected_idx));
            y = [];
            for z = 1:k
